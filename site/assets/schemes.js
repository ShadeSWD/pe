...
(function (global) {
  ...
  /* Вода на этой схеме. Схема — вид СВЕРХУ, и линии поверхности на ней быть
     не может: поверхность занимает всю доску. Без воды обе фигуры висели на
     белом фоне, и схема ничем не показывала, что дело происходит в воде.
     Рисуется тем же PE.swim.waterTop, что и на анимациях раздела, — чтобы
     вода везде выглядела одинаково. Проверка на наличие нужна потому, что
     схемы могут монтироваться и без модуля сцены. */
  PE.scheme('breast-kick-water', () => (PE.swim && PE.swim.waterTop
    ? PE.swim.waterTop(640, 300, { label: false }) : ''));
