...
(function (global) {
  ...
  /* Крен корпуса: тело проворачивается в сторону гребущей руки. Именно крен
     ...
     Плюс — левое плечо вверх, то есть гребёт правая (u ≈ 0,30); минус —
     правое плечо вверх, то есть правая проносится (u ≈ 0,80). Вдох в этой
     анимации приурочен ко второму максимуму: см. фазы crawl-top.
     Амплитуда 38° — обычный крен в кроле (30–45°). */
  const crawlRoll = (u) => 38 * Math.sin(2 * Math.PI * (u - 0.05));
  ...
  PE.reg('crawl-top', {
    w: 640, h: 300, frames: 60, period: 4200,
    phases: [
      { to: 0.36, name: 'Правая гребёт, левая проносится', note: 'Сверху хорошо видно главное: руки работают попеременно и почти всегда одна из них опирается о воду. Кисть гребущей руки идёт не строго назад, а сначала немного внутрь, под тело. Крен в это время наибольший в другую сторону: левое плечо вверху.' },
      { to: 0.62, name: 'Смена крена', note: 'Между гребками тело на мгновение проходит через положение «на груди», и крен меняет знак. Это середина цикла: правая рука заканчивает гребок, левая входит в воду. Крен здесь минимален — и вдоха здесь быть не может.' },
      { to: 0.88, name: 'Поворот головы и вдох', note: 'Правая рука вышла из воды и проносится, правое плечо идёт вверх, крен доходит до наибольшего — 38°. Голова доворачивается вместе с корпусом на ту же сторону, рот выходит в «воздушную ямку» у самой поверхности. Вдох делается именно на максимуме крена: на плечо, которое и так поднимается.' },
    ...
    bg() {
      ...
      return S.waterTop(640, 300, { label: false })
        ...
        + text(512, 152, 'ось тела и движения', C.gray)
        + S.moveArrow(516, 22, 'движение')
        + text(8, 20, 'вид сверху: смотрим на пловца с бортика, сквозь воду', C.water)
        + text(8, 292, 'вид сверху: ось тела лежит на оси движения, выше неё — левая сторона пловца, ниже — правая', C.gray);
    },
    draw(u) {
      /* Голова доворачивается к правому плечу тогда же, когда правая рука
         проносится над водой и правое плечо идёт вверх. Раньше вдох стоял на
         0,56…0,72 — на выходе руки из воды, когда крен ещё только набирался
         (|крен| ≈ 2…30°); теперь окно сдвинуто на 0,70…0,86, то есть на
         максимум крена (u = 0,80, |крен| = 38°). */
      let turn = 0;
      if (u >= 0.60 && u < 0.70) turn = 88 * (u - 0.60) / 0.10;
      else if (u >= 0.70 && u < 0.86) turn = 88;
      else if (u >= 0.86 && u < 0.96) turn = 88 * (1 - (u - 0.86) / 0.10);
      const inhale = u >= 0.70 && u < 0.86;
      ...
      /* near: false — на виде сверху ближней и дальней стороны нет: обе руки
         одинаково далеки от глаза. Пока правая сторона рисовалась тёмной, а
         левая светлой, весь тёмный силуэт (корпус, правая рука, правая нога)
         оказывался ПО ОДНУ сторону от оси, а светлый — по другую, и доска
         читалась как вид сбоку: тело лежит рядом с осью, одна нога уходит
         вбок. Одинаковый тон делает силуэт симметричным относительно оси —
         так же, как на breast-top и legs-breast-top.
         Голова — с макушки: профиль с глазом в этой проекции превращал вид
         сверху в вид сбоку (см. PE.swim.headTop). */
      const o = {
        H, view: 'top', x: 258, y: 158, near: false, face: false,
        where: 'crawl-top',
      };
      const r = PE.figure.solve(pose, o);
      let s = PE.figure.render(r, o);
      s += S.headTop(r, { color: inhale ? C.green : C.ink });
      /* Ось проводится ещё раз, поверх фигуры и полупрозрачно: видно, что
         она проходит сквозь тело, а не рядом с ним. Это и есть главный
         признак вида сверху. */
      s += line(0, 158, 640, 158, C.gray, 0.9,
      ...
      if (inhale) {
        ...
        /* пузырьки выдоха сносит НАЗАД: пловец уходит от них вперёд */
        for (let i = 0; i < 5; i++) {
          const t = (u * 3 + i * 0.2) % 1;
      ...
      s += text(8, 38, 'крен корпуса ' + Math.round(Math.abs(roll)) + '°'
        + (roll > 6 ? ': левое плечо вверх'
          : roll < -6 ? ': правое плечо вверх' : '')
        + (inhale ? ' — на эту сторону и вдох' : ''),
      inhale ? C.green : C.water);
      return s;
    },
    caption: 'Кроль сверху: попеременная работа рук, крен корпуса и поворот головы на вдох. Это та же поза, что и на виде сбоку, — другая проекция, поэтому ось тела здесь лежит на оси движения, а руки уходят в обе стороны от неё. Проверьте по кадрам: вдох приходится на максимум крена, на сторону той руки, которая в этот момент проносится над водой.',
  });
