...
(function (global) {
  ...
  /* Голова на виде СВЕРХУ: макушка, а не профиль. Модель умеет рисовать
     ...
     (face: false), а направление лица показано формой самой головы.

     Как именно. Голова обводится ЗАНОВО поверх круга, нарисованного
     моделью: тот же центр и тот же радиус, но радиус слегка увеличен возле
     направления взгляда — получается небольшой выступ носа на контуре.
     Раньше нос рисовался отдельным закрашенным треугольником у края головы
     и читался как остриё стрелки, а не как часть лица: на этих досках
     треугольники уже заняты стрелками движения и указателями. Выступ на
     контуре спутать с указателем нельзя.

     Центр и направление берутся из точек модели (head и face), а не
     подбираются на глаз: пока лицо в воде, проекция взгляда коротка,
     выступа почти нет и видна только макушка с кружком темени; чем сильнее
     голова довёрнута к плечу, тем заметнее нос. */
  function headTop(res, opt) {
    ...
    /* насколько лицо повёрнуто к нам боком: 0 — смотрит вниз, в воду */
    const k = Math.min(1, el / (R * 0.92));
    const a = el > 1e-6 ? Math.atan2(ey, ex) : 0;
    /* контур головы: окружность с гладким горбом носа в сторону взгляда */
    const N = 96, sig = 0.24, amp = 0.32 * k;
    let d = '';
    for (let i = 0; i <= N; i++) {
      const t = a + (i / N) * Math.PI * 2;
      let dt = t - a;
      if (dt > Math.PI) dt -= Math.PI * 2;
      const rr = R * (1 + amp * Math.exp(-(dt * dt) / (2 * sig * sig)));
      d += `${i ? ' L' : 'M'} ${M.num(hc.x + Math.cos(t) * rr)}`
        + ` ${M.num(hc.y + Math.sin(t) * rr)}`;
    }
    d += ' Z';
    /* та же техника, что и в модели: сначала форма, раздутая обводкой и
       залитая цветом контура, потом заливка — так шов с кругом, который
       нарисовала модель, закрывается сам */
    const g = Math.max(0.8, res.opts.H * 0.0046);
    let s = path(d, ink, g * 2, ink) + path(d, 'none', 0, fill);
    /* кружок темени — пока макушка и правда обращена к нам: когда голова
       довёрнута к плечу, мы смотрим уже не на темя, а на висок */
    if (el < R * 0.55) {
      s += circle(hc.x, hc.y, R * 0.40, 'none', 'rgba(22,22,26,.30)', 1.1);
    }
    return s;
  }
  ...
  PE.reg('breath-cycle', {
    ...
    draw(u) {
      ...
      /* Крен привязан к ФАЗЕ ДЫХАНИЯ, а не к чему придётся: максимум крена
         приходится ровно на середину вдоха (u = 0,68), потому что вдох и
         делается на стороне поднимающегося плеча — доворачивать голову
         больше некуда. Раньше синусоида была сдвинута на 0,32 и максимум
         попадал на середину выдоха: подпись обещала «крен 29°» на выдохе и
         «10°» на вдохе, то есть ровно наоборот. Амплитуда 38° — обычный
         крен в кроле (30–45°). */
      const roll = 38 * Math.sin(2 * Math.PI * (u - 0.43));
      ...
      /* Вторая строка на вдохе прямо связывает крен и дыхание: крен здесь
         наибольший за цикл, и вдох делается именно на поднятое плечо. */
      s += text(14, 58, inhale ? 'голова довёрнута к поднятому плечу: вдох'
